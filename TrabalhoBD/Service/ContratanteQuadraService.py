...
class ContratanteQuadraService:
    ...
    def reservar_horario(self, id_contratante, id_quadra, horario):
        ...
        if cursor is None:
            ...
            sql_time = f"SELECT (Nome) FROM Quadra_Esportiva WHERE id_quadra = '{id_quadra}'"
            cursor.execute(sql_time)
            if cursor is None:
                return f'Não existe Quadra Esportiva com o id = {id_quadra}'
            else:
                sql_insert = f"INSERT INTO Contratante_Quadra (id_contratante, id_quadra, Horario_resevado) values " \
